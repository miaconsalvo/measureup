...
namespace Mystie.UI
{
    public abstract class NavbarUIBase : MonoBehaviour
    {
        ...
        public void SetActiveTab(int i)
        {
            ...
            if (rightBtn != null) rightBtn.interactable = warp || i < TabCount - 1;
        ...
        public int ConstrainIndex(int i)
        {
            if (warp)
            {
                if (i < 0) i = Math.Max(0, TabCount - 1);
                else if (i >= TabCount) i = 0;
            }
            else
            {
                if (i < 0) i = 0;
                else if (i >= TabCount) i = Math.Max(0, TabCount - 1);
            }
        ...
        {
            index = Math.Max(0, Math.Clamp(index, -1, TabCount - 1));
        }
    }
}
